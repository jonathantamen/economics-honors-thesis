...
model1demand <- feols(log_program_expenses ~ gdp_change_percent, data = regression_data)
model1supply <- feols(log_revenues ~ gdp_change_percent, data = regression_data)
etable(model1demand, model1supply)
# 2. Adding yearly data
model2demand <- feols(log_program_expenses ~ gdp_change_percent | year, data = regression_data)
model2supply <- feols(log_revenues ~ gdp_change_percent | year, data = regression_data)
...
## -----------------------------------------------------------------------------
# 4. Adding industry data fixed effects
...
revenue_controlled_demand_model <- feols(log_program_expenses ~ gdp_change_percent + log_revenue | year + state + industry + organization_ein, data = regression_data)

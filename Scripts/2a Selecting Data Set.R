...
# To-Do:
# Schools/hospitals have inelastic demand for their services, so do not respond cyclically or counter-cyclically # nolint: line_length_linter.
...
final_org_data <- org_data |>
  ...
  filter(!(organization_ein %in% bad_org_ids)) |> # filters to organizations that are NOT organizations in bad_eins # nolint: line_length_linter.
  #----------------------------------
